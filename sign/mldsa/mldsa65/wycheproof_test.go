...
// project (testvectors_v1/mldsa_65_verify_test.json). These exercise edge and
...
func TestWycheproofVerify(t *testing.T) {
	buf, err := test.ReadGzip("../testdata/mldsa_65_verify_test.json.gz")
	if err != nil {
		t.Fatal(err)
	}
	...
	t.Logf("ran %d Wycheproof ML-DSA-65 verify vectors", total)
}
